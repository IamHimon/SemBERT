...
class BiGRU(nn.Module):
    ...
    def forward(self, flat_input_ids, embed, num_aspect):  # flat_input_ids.size() = (batch_size*num_aspect, seq_len)
        # flat_input_ids = input_tag_ids.view(-1, input_tag_ids.size(-1))
        # embed = self.embed(flat_input_ids)
        # embed = self.dropout(embed)
        # print("embed", embed.size())

        input = embed.view(flat_input_ids.size(0), embed.size(2), -1)  # [bs*num_aspect, max_len,dim]
        # print("input", input.size())
        self.bigru.flatten_parameters()
        # gru
        gru_out, _ = self.bigru(input)  # [bs*num_aspect, max_len,2*dim]
        # print("gru", gru_out.size())
        gru_out = gru_out.view(-1, num_aspect, flat_input_ids.size(1),
                               2 * self.hidden_size)  # [bs,num_aspect, max_len,2*dim]
        logit = self.fc(gru_out)  # # [bs,num_aspect, max_len, dim]
