...
class BertSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 将hidden_size平均拆分成num_attention_heads，再定义all_head_size
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        # 原文中使用
        attention_probs = self.dropout(attention_probs)  # (batch_size, num_heads, seq_length, seq_length)

        # 用注意力矩阵加权V
        context_layer = torch.matmul(attention_probs, value_layer)  # (batch_size, num_heads, seq_length, depth_v)
        # 把加权后的 V reshape，得到[batch_size, length, hidden_size]
...
class BertOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        """
        hidden_states：Feedforward层输出
        input_tensor：Self-Attention层输出
        """
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)  # 残差实现
        return hidden_states
...
class BertLayer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attention = BertAttention(config)  # Self-Attention层（包括LayerNorm和残差连接）
        self.intermediate = BertIntermediate(config)  # Feedforward层
        self.output = BertOutput(config)  # LayerNorm与残差连接输出层
...
class BertPooler(nn.Module):
    """
    把隐藏层（hidden state)中对应#CLS的token的一条提取出来
    """
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        ...
        self.decoder = nn.Linear(bert_model_embedding_weights.size(1),
                                 ...
                                 bias=False)
        '''
        这是是使用bert.embeddings.word_embeddings的权重，我们当然也可以随机初始化参数。
        这个称作："weight tying" 或者 "joint input-output embedding"，
        常用的策略，可以更快的训练，并可能得到更好的结果。
        paper：
...
class BertPreTrainingHeads(nn.Module):
    ...
    def __init__(self, config, bert_model_embedding_weights):
        ...
        self.seq_relationship = nn.Linear(config.hidden_size, 2)
        '''
        用来把pooled_output也就是对应#CLS#的那一条向量映射为2分类用来进行Next Sentence
        '''
...
class BertForSequenceClassificationTag(BertPreTrainedModel):
    def __init__(self, config, num_labels=2, tag_config=None, gru_config=None):
        super(BertForSequenceClassificationTag, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)  # 基础BERT模型
        self.filter_size = 3
        self.cnn = CNN_conv1d(config, filter_size=self.filter_size)  # subword->原始word
        ...
        if tag_config is not None:
            ...
            self.bi_gru = BiGRU(gru_config)
            self.dense = nn.Linear(tag_config.num_aspect * tag_config.hidden_size, tag_config.hidden_size)  # 最终是展开的
        else:
            hidden_size = config.hidden_size
        use_tag = True
        if use_tag:
            self.last_fc = nn.Linear(config.hidden_size + tag_config.hidden_size,
                                     config.hidden_size + tag_config.hidden_size)
            self.classifier = nn.Linear(config.hidden_size + tag_config.hidden_size, num_labels)
        else:
            self.last_fc = nn.Linear(config.hidden_size, config.hidden_size)
            self.classifier = nn.Linear(hidden_size, num_labels)
        self.apply(self.init_bert_weights)  # 初始化参数

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_end_idx=None, input_tag_ids=None,
                labels=None):
        """
        input_ids,token_type_ids,attention_mask: [bs,max_len]
        input_tag_ids：
        """
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask,
                                       output_all_encoded_layers=False)  # （bs, max_len,bert_dim）
        no_cuda = True
        batch_size, sub_seq_len, dim = sequence_output.size()
        start_end_idx = start_end_idx  # batch * seq_len * (start, end)
        max_seq_len = -1    # 此batch的最大序列长度
        max_word_len = self.filter_size
        for se_idx in start_end_idx:
            num_words = 0
            for item in se_idx:
                if item[0] != -1 and item[1] != -1:
                    num_subs = item[1] - item[0] + 1  # sub—words长度
        ...
        for batch in start_end_idx:
            word_seqs = []  # 合并成原始token采用的sub_words的下标。不足max_word_len用0位置填充，也就是用'CLS'.
            offset = batch_id * sub_seq_len  # 所有index铺平,
            for item in batch:  # raw_words对应的sub_words的所有下标
                if item[0] != -1 and item[1] != -1:
                    subword_ids = list(range(offset + item[0] + 1, offset + item[1] + 2))
                    while len(subword_ids) < max_word_len:  # 0用来做padding成max_word_len长度
                        subword_ids.append(0)
                    word_seqs.append(subword_ids)
            while (len(word_seqs) < max_seq_len):  # 填充长度统一
                word_seqs.append([0 for i in range(max_word_len)])
            # 当max_word_len = 4
            # word_seqs: [[1, 0, 0, 0], [2, 3, 4, 5], [6, 0, 0, 0], [7, 0, 0, 0], [8, 0, 0, 0], [9, 0, 0, 0], [10, 0, 0, 0],
            # [11, 0, 0, 0], [12, 0, 0, 0], [13, 14, 15, 16], [17, 0, 0, 0], [18, 0, 0, 0], [19, 0, 0, 0], [20, 0, 0, 0],
        ...
        batch_start_end_ids = batch_start_end_ids.view(-1) # 完全铺平，下文index_select可以直接取，bs*max_seq_len*max_word_len
        sequence_output = sequence_output.view(-1, dim) # bert的输出也铺平
        sequence_output = torch.cat([sequence_output.new_zeros((1, dim)), sequence_output], dim=0)  # padding的向量
        if not no_cuda:
            batch_start_end_ids = batch_start_end_ids.cuda()
        cnn_bert = sequence_output.index_select(0, batch_start_end_ids)
        cnn_bert = cnn_bert.view(batch_size, max_seq_len, max_word_len, dim)  # reshape回来
        if not no_cuda:
            cnn_bert = cnn_bert.cuda()

        bert_output = self.cnn(cnn_bert, max_word_len)  # [bs, max_seq_len, bert-dim]

        use_tag = True
        use_gru = True
        if use_tag:
            num_aspect = input_tag_ids.size(1)
            input_tag_ids = input_tag_ids[:, :, :max_seq_len]  # 根据max_seq_len截取tag序列（bs,num_aspect,max_seq_len）
            flat_input_tag_ids = input_tag_ids.view(-1, input_tag_ids.size(-1))  # 一二维铺平
            # print("flat_que_tag", flat_input_que_tag_ids.size())
            tag_output = self.tag_model(flat_input_tag_ids, num_aspect)  #tag编码 [bs,num_aspect, max_seq_len, tag-dim]
            if use_gru:
                tag_output = self.bi_gru(flat_input_tag_ids, tag_output, num_aspect) # [bs,num_aspect, max_seq_len, tag-dim]

            # 所有srl序列向量concat一起，[bs, max_seq_len, num_aspect*tag_hidden_size]
            tag_output = tag_output.transpose(1, 2).contiguous().view(batch_size,
                                                                      max_seq_len, -1)
            tag_output = self.dense(tag_output)
            sequence_output = torch.cat((bert_output, tag_output), 2)  # [bs, 51,768+10]
        else:
            sequence_output = bert_output

        first_token_tensor = sequence_output[:, 0]  # first token, [bs, bert-dim+tag_dim],思考：其他方式？

        pooled_output = self.last_fc(first_token_tensor)
        pooled_output = self.activation(pooled_output)
