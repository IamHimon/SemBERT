...
from tag_model.modeling import TagConfig, BiGRUConfig
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, token_tag_sequence_a, token_tag_sequence_b, len_seq_a,
                 len_seq_b, input_tag_ids, input_tag_verbs, input_tag_len, orig_to_token_split_idx, label_id):
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, srl_predictor):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # 加载a序列tokens_a，记下原始完整词在a序列中的下标 tok_to_orig_index_a
        tokens_a_org = tag_sequence[0]
        if len(tag_sequence[1]) > max_aspect:
            max_aspect = len(tag_sequence[1])  # 多少种SRL方案
        tok_to_orig_index_a.append(0)  # 添加[CLS]
        for (i, token) in enumerate(tokens_a_org):  # 遍历每个词，分词之后，记录每个子词原始完整token的位置下标
            sub_tokens = tokenizer.tokenize(token)  # 完整token-> word-piece
            if len(sub_tokens) > 1:
                print(sub_tokens)
            for sub_token in sub_tokens:
                tok_to_orig_index_a.append(i + 1)  # 所有子词原始下标相同
                tokens_a.append(sub_token)      # 记录下子词
        tok_to_orig_index_b = []
        token_tag_sequence_b = None
        # 加载b序列tokens_b，记下原始完整词在b序列中的下标 token_tag_sequence_b
        if example.text_b:
            ...
            # 根据max_seq_length对序列截取，Account for [CLS], [SEP], [SEP] with "- 3"
        ...
        # 构造完整输入序列: "[CLS]" a "[SEP]" b "[SEP]"
        # 构造segment_ids，a中token对应位置为0,b中token对应位置为1
        # 记下a、b的原始序列长度
        ...
        if start_split_ix != -1:  # 加上最后一个
            orig_to_token_split_idx.append((start_split_ix, token_ix))
        while len(orig_to_token_split_idx) < max_seq_length:  # 用(-1, -1)来填充到max_seq_length
            orig_to_token_split_idx.append((-1, -1))

        # 构造input_mask:真实的token为1,padding位置为0;
...
def _truncate_seq_pair(tokens_a, tokens_b, tok_to_orig_index_a, tok_to_orig_index_b, max_length):
    ...
    # 首先截取较长的序列，
    # 思想：如果一个序列比较短，那么它的每个词就可能更重要，比较长序列中的给个词语可能含有更多的information。
    while True:
...
def main():
    ...
    if args.tagger_path != None:
        ...
        srl_predictor = None
    train_examples = None
    # 计算总共需要梯度更新的次数，正常每batch_size并且每gradient_accumulation_steps次 更新一次
    num_train_optimization_steps = None
    if args.do_train:
        train_examples = processor.get_train_examples(args.data_dir)[:100]
    ...
    bigru_config = BiGRUConfig(tag_vocab_size=vocab_size,
                               hidden_size=10,
                               layer_num=1,
                               output_dim=10,
                               dropout_prob=0.1,
    ...
    if args.fp16:
        ...
        # 优化函数有FusedAdam，它是NVIDIA开源面向精简混合精度和分布式训练的Pytorch扩展的优化函数
    ...
    if args.do_train:
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_start_end_idx, all_input_tag_ids,
                                   all_label_ids)
        if args.local_rank == -1:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if args.fp16:
                    ...
                    # 损失放大（Loss Scaling）即使用了混合精度训练，还是会存在无法收敛的情况，原因是激活梯度的值太小，造成了下溢出（Underflow
    ...
    if args.do_eval:
        # for epoch in ["1"]:
        # for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        eval_examples = processor.get_dev_examples(args.data_dir)[:500]
        eval_features = convert_examples_to_features(
            eval_examples, label_list, args.max_seq_length, tokenizer, srl_predictor=srl_predictor)
        eval_features = transform_tag_features(args.max_num_aspect, eval_features, tag_tokenizer,
                                               args.max_seq_length)
        ...
        # epoch = 1
        output_model_file = os.path.join(args.output_dir, "2_pytorch_model.bin")
        model_state_dict = torch.load(output_model_file)
        predict_model = BertForSequenceClassificationTag.from_pretrained(args.bert_model,
                                                                         state_dict=model_state_dict,
                                                                         num_labels=num_labels,
    ...
    if args.do_predict:
        ...
        model_state_dict = torch.load(output_model_file)
        predict_model = BertForSequenceClassificationTag.from_pretrained(args.bert_model, state_dict=model_state_dict,
                                                                         num_labels=num_labels, tag_config=tag_config)
        predict_model.to(device)
        predict_model.eval()
        predictions = []
